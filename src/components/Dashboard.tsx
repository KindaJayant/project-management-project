    ...
    <div className="space-y-8 p-2">
      <header className="flex justify-between items-end">
        <div>
          ...
          <p className="text-[#BABABA] mt-1">Real-time analytical metrics for Strategic Digital Overhaul.</p>
        </div>
        <div className="flex gap-3">
           <div className="glass-card !py-2 !px-4 flex items-center gap-2">
            <Clock size={16} className="text-[#FF6D29]" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          ...
          >
            ...
            <div className="absolute -right-4 -bottom-4 w-24 h-24 bg-[#FF6D29]/5 rounded-full blur-2xl group-hover:bg-[#FF6D29]/10 transition-all" />
          </motion.div>
        ))}
      </div>
        ...
        <div className="lg:col-span-2 glass-panel p-8">
          <div className="flex justify-between items-center mb-10">
            ...
            <div className="flex gap-2">
              <span className="px-3 py-1 bg-white/5 rounded-full text-[10px] text-[#BABABA] uppercase font-bold tracking-widest">Q1 2026</span>
            </div>
          </div>
            ...
            {[65, 45, 78, 52, 85, 42, 60].map((h, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-3">
                <motion.div 
                  ...
                  className={`w-full max-w-[40px] rounded-t-lg transition-all duration-500 ${i === 4 ? 'bg-[#FF6D29] accent-glow' : 'bg-white/10'}`}
                />
                <span className="text-[10px] text-[#BABABA] font-bold">W-0{i+1}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            {recentTasks.map((task, i) => (
              <div key={i} className="flex items-start gap-4">
                <div className={`w-2 h-2 rounded-full mt-2 ${
                  task.priority === 'High' ? 'bg-[#FF6D29]' : 
                  task.priority === 'Medium' ? 'bg-[#BABABA]' : 'bg-white/10'
                }`} />
                <div className="flex-1">
                  <h4 className="text-sm font-semibold">{task.name}</h4>
                  <p className="text-xs text-[#BABABA]">{task.owner} • {task.status}</p>
                </div>
              </div>
            ))}
